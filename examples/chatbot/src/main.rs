...
impl Config {
    pub fn load(path: &std::path::Path) -> Result<Self, color_eyre::Report> {
...
async fn main() -> Result<(), color_eyre::Report> {
    ...
    tracing_subscriber::fmt::fmt()
        ...
        .init();
    _ = dotenvy::from_path(std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join(".env"))?;
    let opts = Cli::parse();
...
impl Bot {
    pub async fn start(&self) -> Result<(), color_eyre::Report> {
        ...
        websocket
            .run(|e, ts| async { self.handle_event(e, ts).await })
            .await?;
        Ok(())
    }

    pub async fn handle_event(&self, event: Event, timestamp: twitch_api::types::Timestamp) {
